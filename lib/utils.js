...
exports.interceptors = require('./deps/interceptors').interceptors;
...
exports.adapterFunChain = function (name, callback) {
...
exports.adapterFun = function (name, callback) {
  return exports.toPromise(exports.getArguments(function (args) {
    ...
    if (!this.taskqueue.isReady) {
      ...
      var chain         = [self[name], null];

      exports.interceptors.forEach(function (interceptor) {
        if (interceptor.request || interceptor.requestError) {
          var reqFn     = interceptor.request,
              reqErrFn  = interceptor.requestError;

          chain.unshift(reqFn, reqErrFn);
        }
        if (interceptor.response || interceptor.responseError) {
          var resFn     = interceptor.response,
              resErrFn  = interceptor.responseError;

          chain.push(resFn, resErrFn);
        }
      });

      // Now build the chain

      // The first entries of the chain are where we'll start
      var chain0      = chain.shift(),
          chain1      = chain.shift();

      var promise = new exports.Promise(function (fulfill, reject) {
        self.taskqueue.addTask(function (failed) {
          if (failed) {
            reject(chain1 ? chain1.apply(self, failed) : failed);
          } else {
            fulfill(chain0.apply(self, args));
          }
        });
      });

      var forwardArguments = function (fn) {
        return exports.toPromise(exports.getArguments(function (args) {
          console.log('forwardArguments called', args);
          return new exports.Promise(function (fulfill, reject) {
            self.taskqueue.addTask(function (failed) {
              console.log('in taskqueue', failed);
              if (failed) {
                reject(failed);
              } else {
                return fulfill(fn.apply(self, args));
              }
            });
          });

          console.log('added', self.taskqueue);
          // var resolve = args.pop();
          // // resolve.apply(this, function () {
      ...
      while (chain.length) {
        ...
        promise = promise
          .then(forwardArguments(thenFn), forwardArguments(rejectFn));
      }
